...
export function HouseTile({ entries }: { entries: HouseEntry[] }) {
  ...
  return (
    <div id={id} data-pick="0" className="house-tile bg-[#111111] text-[#F9F9F7] p-7">
      <style>{`
      ...
      {entries.map((entry) => (
        <article key={entry.href}>
          ...
          <Link href={entry.href} className="block mt-4" tabIndex={-1} aria-hidden="true">
            <div
              className="ca-photo relative w-full overflow-hidden bg-black/30 aspect-[16/10]"
          ...
          <h3 className="type-rail-h mt-4 text-white">{entry.headline}</h3>
          <p className="font-serif text-[15px] leading-[1.5] text-[#F9F9F7]/80 mt-2">{entry.body}</p>
          <Link
            href={entry.href}
            className="block w-full text-center bg-[#C5A059] text-black font-serif text-[12px] leading-none font-black uppercase tracking-[0.12em] px-5 py-3 mt-5 hover:bg-white transition-colors"
          >
            {entry.cta}
          </Link>
        </article>
