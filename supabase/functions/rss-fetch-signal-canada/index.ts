...
// rss-fetch-signal-canada v1 — Signal Canada vertical
const VERTICAL = 'Signal Canada';
const JOB_NAME = 'rss-fetch-signal-canada';
...
async function fetchFeed(url: string): Promise<string> { const resp = await fetch(url, { headers: { 'User-Agent': 'CityAge/SignalCanada RSS 1.0' }, signal: AbortSignal.timeout(10000) }); if (!resp.ok) throw new Error(`HTTP ${resp.status}`); return await resp.text(); }
...
async function synopsizeArticle(title: string, summary: string, sourceName: string, category: string, apiKey: string, recentTitles: string[]): Promise<{synopsis: string, relevance: number, duplicate: boolean}> {
  ...
  try {
    const resp = await fetch('https://api.anthropic.com/v1/messages', { method: 'POST', headers: { 'Content-Type': 'application/json', 'x-api-key': apiKey, 'anthropic-version': '2023-06-01' }, body: JSON.stringify({ model: 'claude-haiku-4-5-20251001', max_tokens: 300, messages: [{ role: 'user', content: `Reputation intelligence analyst for Signal Canada — tracking global perception of Canada as investment destination.\n\nSource: ${sourceName} (${category})\nTitle: ${title}\nSummary: ${summary}\n${dedupeCtx}\n\nJSON only: {"synopsis":"2-3 sentences. WHO said WHAT about Canada.","relevance":7,"duplicate":false}\n\n8-10: Named global leader/investor statement about Canada, FDI with dollar amount, rating action\n5-7: Policy analysis, competitiveness comparison, trade data\n3-4: General Canadian business\n1-2: No investment perception relevance` }] }) });
...
Deno.serve(async (req: Request) => {
  ...
  const { data: recent } = await supabase.from('rss_articles').select('title').gte('created_at', new Date(Date.now() - 24*60*60*1000).toISOString()).eq('is_synopsized', true).eq('vertical', VERTICAL);
  processedTitles.push(...(recent ?? []).map((a: any) => a.title));
